...
def welcome(message):
	...
	bot.send_message(message.chat.id, "Привет тебе от капибары, {0.first_name}!".format(message.from_user, bot.get_me()),
...
def lalala(message):
	if message.chat.type == 'private':
		if message.text == '🧡 Мой репозиторий':
			bot.send_message(message.chat.id, 'https://github.com/Denis-N-QA')
		elif message.text == '😋 Написать мне в личку':
			bot.send_message(message.chat.id, 'https://t.me/Denis_Nigmatylin')
		else:
